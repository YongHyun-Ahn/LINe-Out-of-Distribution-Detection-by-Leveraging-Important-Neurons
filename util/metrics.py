...
def print_results(results, in_dataset, out_dataset, name, detail, method):
    ...
    print(' OOD detection method: ' + method + detail)
    for mtype in mtypes:
...
def print_all_results(results, datasets, method, detail, in_dataset):
    mtypes = ['FPR', 'AUROC', 'AUIN']
    avg_results = compute_average_results(results)
    i = 0
    txt = np.zeros((len(results)+1,len(mtypes)))
    print(' OOD detection method: ' + method +'_'+ detail)
    print('             ', end='')
    for mtype in mtypes:
        print(' {mtype:6s}'.format(mtype=mtype), end='')
    for result, dataset in zip(results,datasets):
        ...
        txt[i,:] = [100.*result['FPR'],100.*result['AUROC'],100.*result['AUIN']]
        i += 1
    ...
    txt[i,:] = [100.*avg_results['FPR'],100.*avg_results['AUROC'],100.*avg_results['AUIN']]
    print('')
...
def compute_traditional_ood(base_dir, in_dataset, out_datasets, method, detail, name):
    # print('Natural OOD')
    # print('nat_in vs. nat_out')

    known = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/nat/in_scores_{method}_{detail}.txt'.format(base_dir=base_dir, in_dataset=in_dataset, method=method, name=name, detail=detail))
    ...
    for out_dataset in out_datasets:
        novel = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/nat/{out_dataset}/out_scores_{method}_{detail}.txt'.format(base_dir=base_dir, in_dataset=in_dataset, method=method, detail=detail, name=name, out_dataset=out_dataset))
    ...
    print_all_results(all_results, out_datasets, method, detail, in_dataset)
    # avg_results = compute_average_results(all_results)
    # print_results(avg_results, in_dataset, "All", name, method)


def compute_stat(base_dir, in_dataset, out_datasets, method, detail, name):
    # print('Natural OOD')
    # print('nat_in vs. nat_out')

    known = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/nat/in_scores_{method}_{detail}.txt'.format(base_dir=base_dir, in_dataset=in_dataset, method=method, detail=detail, name=name))
...
def compute_in(base_dir, in_dataset, method, detail, name):

    known_nat = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/nat/in_scores_{method}.txt_{detail}'.format(base_dir=base_dir, in_dataset=in_dataset, method=method, detail=detail, name=name))
    ...
    known_nat_label = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/nat/in_labels_{method}_{detail}.txt'.format(base_dir=base_dir, in_dataset=in_dataset, method=method, detail=detail, name=name))
