...
def get_shap_energy_score(inputs, model, method_args):

    inputs = Variable(inputs, requires_grad = True)
    outputs = model(inputs)
    
    scores = outputs.squeeze()
    scores = torch.logsumexp(scores, dim=1).detach().cpu().numpy()
    return scores
...
def get_score(inputs, model, method, method_args, raw_score=False):
    ...
    elif method in {'taylor', 'taylor_abs', 'integrad', 'integrad_abs'}:
        scores = get_shap_energy_score(inputs, model, method_args)
    else:
